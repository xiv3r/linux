...
static const struct regulator_bulk_data s6e8aa5x01_ams497hy01_supplies[] = {
	{ .supply = "vdd3" },
	{ .supply = "vci" },
};
...
static int s6e8aa5x01_ams497hy01_on(struct s6e8aa5x01_ams497hy01 *ctx)
{
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xb2, 0x00, 0x0f, 0x00, 0x0f);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xb6, 0xbc, 0x0f);
...
static int s6e8aa5x01_ams497hy01_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(s6e8aa5x01_ams497hy01_supplies), ctx->supplies);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(s6e8aa5x01_ams497hy01_supplies), ctx->supplies);
		return ret;
	}
...
static int s6e8aa5x01_ams497hy01_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx = devm_drm_panel_alloc(dev, struct s6e8aa5x01_ams497hy01, panel,
				   &s6e8aa5x01_ams497hy01_panel_funcs,
				   DRM_MODE_CONNECTOR_DSI);
	if (IS_ERR(ctx))
		return PTR_ERR(ctx);

	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(s6e8aa5x01_ams497hy01_supplies),
					    s6e8aa5x01_ams497hy01_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
